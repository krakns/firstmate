...
While `state/.afk` exists that verdict short-circuits ahead of the model dispatch, because in away mode the daemon owns the watcher whatever the harness is.
It requires a live identity-matched daemon through `fm_afk_daemon_owns_supervision` before it looks at the beacon at all, so away mode flagged with no live daemon is the `no-afk-daemon` reason at every beacon age, both inside the blind window where the reaped daemon's watcher child left a beacon still fresh and long after that beacon decayed.
Only a daemon that still owns the home can then fail on a stale beacon alone.
With `state/.afk` absent the model dispatch below is unchanged.
...
Under every persistent-watcher harness a live identity-matched watcher with a fresh beacon is still required, so the pull guard keeps the same strict semantics there, except while away mode is flagged and the away-mode short-circuit above owns the verdict.
Its banner names the true failing condition, either a missing live away-mode daemon, a missing live watcher process, or a genuinely stale beacon with its real age, and keys the once-per-episode dedup on that condition rather than the beacon mtime.
The away-mode case gets its own header, `AWAY MODE FLAGGED, BUT NO SUPERVISOR IS RUNNING`, because a lingering flag with nothing behind it is worse than supervision being plainly off.
...
It covers the away-mode short-circuit on both sides of the blind window, alarming with `AWAY MODE FLAGGED, BUT NO SUPERVISOR IS RUNNING` when the flag is set with no live daemon whether the beacon is still fresh or long stale, and staying silent when a live identity-matched daemon owns the home.
It also covers the banner's repair line agreeing with that header, since the repair line must not tell the reader away mode owns supervision under a header saying no supervisor is running.
...
`tests/fm-supervision-instructions.test.sh` covers recovery-line ownership, pi-signed's identity-preserving reuse of Pi's protocol, and the caller-supplied away-mode half-state signal, which reports no supervisor only when the caller answers that question and otherwise keeps the ordinary away-mode wording.

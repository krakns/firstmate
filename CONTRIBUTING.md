...
Because that binding is to the head commit and not to the branch, any commit that reaches the PR branch outside the gate - a hand-pushed fixup, an amend, or a rebase - leaves the PR body attesting the previous head, and the check fails with `Pipeline attestation head_sha does not match the current PR head`.
